  ...
  clone(object) {
    return React.Children.map(object, (option) => {
      if (option.type.isOption) {
        ...
        return (
          <MockOption
            onMouseDown={(event) => { console.log('[Search Menu] - Option - OnMouseDown'); event.preventDefault(); event.nativeEvent.stopImmediatePropagation(); event.stopPropagation(); this.downOption = option; }}
            onMouseUp={event => this.handleOptionClick(event, option)}
          >
